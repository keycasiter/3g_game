	...
	for _, general := range allGenerals {
		if util.DebuffEffectWrapSet(ctx, general, consts.DebuffEffectType_LaunchStrategyDamageDeduce, &vo.EffectHolderParams{
			...
			util.TacticsTriggerWrapRegister(general, consts.BattleAction_BeginAction, func(params *vo.TacticsTriggerParams) *vo.TacticsTriggerResult {
				...
				if revokeRound == consts.Battle_Round_Fourth {
					if util.DebuffEffectWrapRemove(ctx, revokeGeneral, consts.DebuffEffectType_LaunchStrategyDamageDeduce, g.Id()) {
						hlog.CtxInfof(ctx, "[%s]造成谋略伤害提升30%%",
							general.BaseInfo.Name,
						)
					}
				}
