...
func RecWarBookList(ctx context.Context, c *app.RequestContext) {
	var err error
	var req api.RecWarBookListRequest
	...
	hlog.CtxInfof(ctx, "RecWarBookList Req:%s", util.ToJsonString(ctx, req))

	resp, err := battle.NewRecWarBookListLogic(ctx, req).Handle()
	if err != nil {
		hlog.CtxErrorf(ctx, "RecWarBookListLogic handle err:%v", err)
